...
import { Check, CtaButton } from './pages/Shell.jsx';
...
// Live pricing (single source of truth = the HRMS console), falls back to content.
...
export function usePricing() {
  ...
  useEffect(() => {
    let alive = true;
    fetch(PRICING_API).then((r) => r.json()).then((j) => { if (alive && Array.isArray(j?.plans)) setApiPlans(j.plans); }).catch(() => {});
    return () => { alive = false; };
  }, []);
  return apiPlans;
}

// Re-run the scroll-reveal observer when a section mounts (for standalone pages).
        ...
        <div className="device-row reveal" role="img" aria-label="Vorkhive HRMS app screens: Command Centre dashboard and Staff Directory.">
          ...
          <div className="phone"><div className="screen">
            ...
            <div className="app-scroll">
              ...
              <div className="sd-search"><span>Search name, role, department…</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#B8B4D0" strokeWidth="2" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="M21 21l-4-4" /></svg></div>
              ...
              <div className="emp"><div className="eh"><span className="av">WJ</span><div><div className="en">Wei Jing Lim</div><div className="er">Senior Software Engineer</div></div></div><div className="ed"><span>DEPT</span><span className="badge">ENGINEERING</span></div><div className="ed"><span>EMPLOYEE ID</span><b>EMP-0001</b></div><div className="ed"><span>CONTRACT</span><b>FULL TIME</b></div></div>
              <div className="emp"><div className="eh"><span className="av">PN</span><div><div className="en">Priya Nair</div><div className="er">Frontend Engineer</div></div></div><div className="ed"><span>DEPT</span><span className="badge">ENGINEERING</span></div><div className="ed"><span>EMPLOYEE ID</span><b>EMP-0002</b></div></div>
            </div>
            <div style={{ position: 'relative' }}><div className="float-chat" aria-hidden="true">💬</div></div>
          </div></div>
        </div>
...
export function LogosSection() {
  ...
  return (
    <section className="logos reveal" aria-label="Customers" style={{ paddingTop: '48px' }}>
      <div className="wrap"><p>{logos.caption}</p><div className="logo-row">{logos.items.map((l, i) => <span key={i}>{l}</span>)}</div></div>
    </section>
  );
}
...
export function IntroSection() {
  ...
  return (
    <section className="intro reveal" aria-labelledby="intro-h2">
      <div className="wrap"><h2 id="intro-h2">{intro.heading}</h2><p>{intro.body1}</p><p>{intro.body2}</p></div>
    </section>
  );
}
  ...
  return (
    <section style={{ padding: '64px 0' }} aria-labelledby="prob-h2">
      <div className="problem">
        <div className="sec-head reveal"><p className="eyebrow">{problem.eyebrow}</p><h2 id="prob-h2">{problem.heading}</h2><p>{problem.sub}</p></div>
        <div className="pain-grid">{problem.items.map((it, i) => (<article className="pain reveal" key={i}><div className="ic" aria-hidden="true">{it.icon}</div><h3>{it.title}</h3><p>{it.text}</p></article>))}</div>
      </div>
    </section>
  );
}

export function PillarsSection({ asPage }) {
  const { pillars } = useContent();
  const H = asPage ? 'h1' : 'h2';
  return (
    <section id="platform" aria-labelledby="plat-h2">
      <div className="wrap">
        <div className="sec-head reveal"><p className="eyebrow">{pillars.eyebrow}</p><H id="plat-h2">{pillars.heading}</H><p>{pillars.sub}</p></div>
        <div className="pillars">{pillars.items.map((p, i) => (
          <article className="pillar reveal" key={i}><span className="tag">{p.tag}</span><div className="pic" aria-hidden="true">{p.icon}</div><h3>{p.title}</h3><p>{p.text}</p><ul>{p.bullets.map((b, j) => <li key={j}><Check />{b}</li>)}</ul></article>
        ))}</div>
      </div>
    </section>
  );
...
export function FeaturesSection({ asPage }) {
  const { features } = useContent();
  const H = asPage ? 'h1' : 'h2';
  return (
    <section id="features" style={{ paddingTop: '40px' }} aria-labelledby="feat-h2">
      <H id="feat-h2" className="wrap" style={{ textAlign: 'center', fontSize: 'clamp(1.8rem,3.4vw,2.6rem)', marginBottom: '50px' }}>{features.heading}</H>
      <div className="wrap">
        <div className="feature-row reveal">
          <div className="ftext"><p className="eyebrow">{features.items[0].eyebrow}</p><h3>{features.items[0].title}</h3><p>{features.items[0].text}</p><ul>{features.items[0].bullets.map((b, j) => <li key={j}><Check s={18} sw={2.5} />{b}</li>)}</ul></div>
        ...
        <div className="feature-row rev reveal">
          <div className="ftext"><p className="eyebrow">{features.items[1].eyebrow}</p><h3>{features.items[1].title}</h3><p>{features.items[1].text}</p><ul>{features.items[1].bullets.map((b, j) => <li key={j}><Check s={18} sw={2.5} />{b}</li>)}</ul></div>
          <div className="fvisual" style={{ gap: '14px' }} role="img" aria-label="Compliance: CPF automatic, statutory leave compliant, IRAS export ready.">
            <div className="mini-card"><div className="av" style={{ background: '#11243F', color: '#C9A24E', fontSize: '.7rem' }} aria-hidden="true">CPF</div><div className="mc"><b>CPF contribution</b><span>Employer + employee</span></div><span className="amt">Auto</span></div>
            <div className="mini-card"><div className="av" style={{ background: '#11243F', color: '#C9A24E', fontSize: '.62rem' }} aria-hidden="true">MOM</div><div className="mc"><b>Statutory leave</b><span>Entitlements aligned</span></div><span className="pill">Compliant</span></div>
            <div className="mini-card"><div className="av" style={{ background: '#11243F', color: '#C9A24E', fontSize: '.6rem' }} aria-hidden="true">IRAS</div><div className="mc"><b>Year-end filing</b><span>IR8A export ready</span></div><span className="pill">Ready</span></div>
          </div>
        </div>
      </div>
    </section>
...
export function MetricsSection() {
  ...
  return (
    <section style={{ padding: '30px 0 70px' }} aria-label="Key results">
      <div className="wrap"><div className="metrics reveal">{metrics.map((m, i) => <div className="metric" key={i}><div className="num">{m.num}</div><div className="lab">{m.lab}</div></div>)}</div></div>
    </section>
  );
}
...
export function StepsSection() {
  ...
  return (
    <section aria-labelledby="steps-h2">
      <div className="wrap">
        <div className="sec-head reveal"><p className="eyebrow">{steps.eyebrow}</p><h2 id="steps-h2">{steps.heading}</h2><p>{steps.sub}</p></div>
        <div className="steps">{steps.items.map((s, i) => <article className="step reveal" key={i}><div className="n">{i + 1}</div><h3>{s.title}</h3><p>{s.text}</p></article>)}</div>
      </div>
    </section>
  );
}

export function CustomersSection({ asPage }) {
  const { testimonials } = useContent();
  const H = asPage ? 'h1' : 'h2';
  return (
    <section id="testimonials" style={{ paddingTop: '30px' }} aria-labelledby="test-h2">
      <div className="wrap">
        <div className="sec-head reveal"><p className="eyebrow">{testimonials.eyebrow}</p><H id="test-h2">{testimonials.heading}</H></div>
        <div className="tgrid">{testimonials.items.map((t, i) => (
          <figure className="tcard reveal" key={i}><div className="stars" aria-label="5 out of 5 stars">★★★★★</div><blockquote>{t.quote}</blockquote><figcaption className="tperson"><div className="av" aria-hidden="true">{t.initials}</div><div><b>{t.name}</b><span>{t.role}</span></div></figcaption></figure>
        ))}</div>
      </div>
    </section>
  );
...
export function PricingSection({ asPage }) {
  const { pricing } = useContent();
  const apiPlans = usePricing();
  const plans = apiPlans ? apiPlans.map(toMarketingPlan) : pricing.plans;
  const H = asPage ? 'h1' : 'h2';
  return (
    <section id="pricing" aria-labelledby="price-h2">
      <div className="wrap">
        <div className="sec-head reveal"><p className="eyebrow">{pricing.eyebrow}</p><H id="price-h2">{pricing.heading}</H><p>{pricing.sub}</p></div>
        <div className="pricing-grid">{plans.map((p, i) => (
          <article className={`price reveal${p.featured ? ' feat' : ''}`} key={i}>
            ...
            <h3 className="pname">{p.name}</h3><div className="pdesc">{p.desc}</div><div className="amt">{p.price}<small>{p.period}</small></div>
...
export function FaqSection({ asPage }) {
  const { faq } = useContent();
  const apiPlans = usePricing();
  const items = apiPlans ? faq.items.map((it) => (/how much/i.test(it.q || '') ? { ...it, a: costAnswer(apiPlans) } : it)) : faq.items;
  const H = asPage ? 'h1' : 'h2';
  return (
    <section id="faq" aria-labelledby="faq-h2" style={{ paddingTop: '30px' }}>
      <div className="wrap">
        <div className="sec-head reveal"><p className="eyebrow">{faq.eyebrow}</p><H id="faq-h2">{faq.heading}</H><p>{faq.sub}</p></div>
        <div className="faq-list reveal">{items.map((f, i) => (<details className="faq" key={i}><summary>{f.q}</summary><div className="fa">{f.a}</div></details>))}</div>
      </div>
    </section>
  );
}
  ...
  return (
    <section id="demo" style={{ padding: '40px 0 90px' }} aria-labelledby="cta-h2">
      <div className="final reveal">
        <h2 id="cta-h2">{finalCta.heading}</h2><p>{finalCta.sub}</p>
        <div className="hero-cta">
