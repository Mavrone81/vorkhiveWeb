...
export default function FloatingContacts() {
  ...
  const wa = (ct.whatsapp || '6587007621').replace(/\D/g, '');
  const email1 = ct.email1 || 'samuel@vorkhive.com';
  const email2 = ct.email2 || 'enquires@vorkhive.com';

  const waText = encodeURIComponent("Hi Vorkhive, I'd like to know more about your HRMS.");
  const mailSubject = encodeURIComponent('Vorkhive enquiry');
  const mailBody = encodeURIComponent('Hi Vorkhive team,\n\nI would like to know more about your HRMS.\n\n');
  const mailHref = `mailto:${email1}${email2 ? `?cc=${email2}&` : '?'}subject=${mailSubject}&body=${mailBody}`;

  const actions = [
    {
      key: 'whatsapp', label: 'WhatsApp', color: '#25D366', external: true,
    ...
    {
      key: 'call', label: 'Call us', color: '#5B3DF5', external: false,
      href: `tel:${phone}`,
    ...
    {
      key: 'email', label: 'Email us', color: '#334155', external: false,
      href: mailHref,
      icon: (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" /></svg>),
    },
  ];
